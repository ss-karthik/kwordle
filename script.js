...
function playTurn(input)
{
    ...
    if(guessed == true)
    {
        gameinfo.innerHTML = "You have Guessed the word!";
    }
    else if(tries==maxTries)
    {
        let string = "The answer is: " + ans + "<br>Game Over!"   
...
{   
    ...
    let cakewalk = document.createElement('button');
    cakewalk.innerHTML = "Cakewalk";
    cakewalk.addEventListener("click", ()=> {
        setMode(10);
    });
    ...
    butdiv.classList.add("dialog-buttons");
    dialog.appendChild(text);
    butdiv.appendChild(cakewalk);
    butdiv.appendChild(easy);
    butdiv.appendChild(normal);
    butdiv.appendChild(hard);
    dialog.appendChild(butdiv);


    let box = document.getElementById('content');
    let title = document.createElement("h1");
    title.innerHTML = "Kwordle";
    let subtitle = document.createElement("h3");
    let str = "Each guess gets you closer...";
...
function regenGrid(grid, x)
{
    ...
    for(let i=0; i<maxTries; i++)
    {
        ...
        for(let j=0; j<10; j=j+2)
        {
            ...
            if(currentWord[j]!=undefined)
            {
                cell.innerHTML = currentWord[j];
                ch = currentWord[j+1];
                if(ch=='G')
                {
